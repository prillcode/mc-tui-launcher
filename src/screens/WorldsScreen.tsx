      ...
      <Centered maxWidth={96}>
                    ...
                    <For each={worlds()}>
                      {(world, i) => (
                        <WorldRow
                          ...
                          world={world}
                          selected={selected() === i()}
                          versionMismatch={versionMismatch(world)}
                          instanceVersionId={instance()?.versionId}
                          pendingDelete={pendingDelete() === world.folder}
                        />
                      )}
                    </For>
                  </scrollbox>
        ...
        <Show when={mode() === "worlds"}>
          <WorldDetails
            world={selectedWorld()}
            stats={selectedStats()}
            versionMismatch={versionMismatch(selectedWorld())}
            instanceVersionId={instance()?.versionId}
            backupsDir={launcherService.worldsBackupsDir(instanceId() ?? "")}
            running={running()}
            truncate={truncate}
          />
        </Show>
